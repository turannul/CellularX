...
- (NSArray *)specifiers
{
    if(_specifiers == nil) {
        ...
        PSSpecifier* footSpecifier = [PSSpecifier emptyGroupSpecifier];
                [footSpecifier setProperty:@"© 2020-2024 TuranUl" forKey:@"footerText"];
                [specifiers addObject:footSpecifier];
        PSSpecifier *applyButton = [PSSpecifier preferenceSpecifierNamed:@"Apply Changes" target:self set:nil get:nil detail:nil cell:[PSTableCell cellTypeFromString:@"PSButtonCell"] edit:nil];
        applyButton->action = @selector(apply);
        [specifiers addObject:applyButton];
...
-(void)apply
{
    [self RunCMD:@"killall -9 CommCenter" WaitUntilExit:YES];
}
//=============================================================================
@end
